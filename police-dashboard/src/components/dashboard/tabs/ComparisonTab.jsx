...
const ComparisonTab = ({ data = [], monthNames = ["ม.ค.", "ก.พ."], isPrint = false }) => {
    return (
            ...
            <div className={`bg-white rounded-2xl md:rounded-[2.5rem] shadow-xl border border-slate-200 ${isPrint ? 'p-4 md:p-6' : 'p-4 md:p-8'}`}>
                <div className={`flex items-center justify-between ${isPrint ? 'mb-4' : 'mb-8'}`}>
                    <h3 className="text-3xl font-bold text-[#1c2e4a] flex items-center gap-3">
                        ...
                        เปรียบเทียบสถิติการจับกุม (2 เดือนย้อนหลัง)
                    </h3>
                </div>
                <div className={`w-full ${isPrint ? 'h-[600px] md:h-[650px]' : 'h-[600px] md:h-[850px]'}`}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart layout="vertical" data={data} margin={{ top: 20, right: 80, left: 40, bottom: 20 }}>
                            ...
                            <Bar dataKey="month2" name={monthNames[1]} fill="#004aad" radius={[0, 8, 8, 0]} barSize={40} label={{ position: 'right', fill: '#1e3a8a', fontSize: 24, fontWeight: 'bold', formatter: (val) => val.toLocaleString() }} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
