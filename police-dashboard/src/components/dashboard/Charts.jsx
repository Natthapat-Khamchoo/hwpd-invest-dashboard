        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 20, right: 0, left: -20, bottom: 20 }}>
            ...
            <Bar dataKey="value" radius={[6, 6, 2, 2]} maxBarSize={50} onClick={onBarClick} cursor="pointer">
              ...
              <LabelList dataKey="value" position="top" fill="#ffffff" fontSize={11} fontWeight="bold" style={{ textShadow: '0 0 10px rgba(0,0,0,0.8)' }} />
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
...
export const MonthlyBarChart = ({ data, year, onYearChange }) => (
  <div className="mt-6 glass-liquid p-4 sm:p-6 rounded-xl transition-all duration-300">
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
      ...
      <div className="flex items-center space-x-2 bg-slate-900/50 p-1.5 rounded-lg border border-slate-700/50 backdrop-blur-sm">
  ...
  return (
    <div className="glass-liquid p-4 sm:p-6 rounded-xl transition-all duration-300">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-64 flex-shrink-0 bg-slate-900/60 p-5 rounded-xl border border-slate-700/50 flex flex-col justify-center shadow-inner relative overflow-hidden group">
          ...
          <h4 className="text-sm font-semibold text-slate-300 mb-4 border-b border-slate-700 pb-2 relative z-10">สรุปผลการเปรียบเทียบ</h4>

          <div className="space-y-4 relative z-10">
            <div>
              <p className="text-xs text-blue-400 mb-0.5">รวมช่วงเวลาที่ 1</p>
              <div className="text-xl font-bold text-white drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]">{totals.total1} <span className="text-sm text-slate-500 font-normal">คดี</span></div>
            </div>
            <div>
              <p className="text-xs text-yellow-400 mb-0.5">รวมช่วงเวลาที่ 2</p>
              <div className="text-xl font-bold text-white drop-shadow-[0_0_8px_rgba(234,179,8,0.5)]">{totals.total2} <span className="text-sm text-slate-500 font-normal">คดี</span></div>
            </div>
            ...
            <div className="pt-2 border-t border-slate-700/50">
              ...
              <div className={`flex items-baseline space-x-2 ${totals.diff >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                <div className="text-3xl font-bold drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]">
